...
**Описание:** Образовательный курс с уроками
...
class Course(BaseModel):
    ...
    # Автор и владелец
    owner: models.ForeignKey            # Создатель курса (User)
    author_name: models.CharField       # Имя автора для отображения
    
    # Ценообразование
    price: models.DecimalField          # Цена курса
    currency: models.CharField          # Валюта (RUB, USD, EUR)
    ...
    # Категоризация
    category: models.CharField          # Категория курса
    tags: models.ManyToManyField       # Теги для поиска (опционально)
    
    # Статистика
    students_count: models.IntegerField # Количество учеников
    rating: models.DecimalField         # Средний рейтинг
...
**Методы:**
- `is_published()` - проверка публикации
- `get_lessons_count()` - количество уроков
- `get_duration()` - общая длительность
- `enroll_student(user)` - записать студента
- `calculate_rating()` - пересчитать рейтинг

---

### 2. Lesson (Урок)

**Описание:** Отдельный урок в курсе
...
class Lesson(BaseModel):
    ...
    duration: models.DurationField      # Длительность видео
    transcript: models.TextField        # Текстовая расшифровка (опционально)
    
    # Материалы
    materials: models.FileField         # Дополнительные материалы
    
    # Доступность
    is_preview: models.BooleanField     # Доступен для предпросмотра
...
- `get_prev_lesson()` - предыдущий урок
- `mark_completed(user)` - отметить просмотренным

---

### 3. Subscription (Подписка на курс)

**Описание:** Связь между пользователем и курсом
...
class Subscription(BaseModel):
    ...
    expires_at: models.DateTimeField    # Дата окончания (null=бессрочно)
    ...
    # Оплата
    payment: models.ForeignKey          # Связанный платеж
...
class Payment(BaseModel):
    ...
    course: models.ForeignKey           # Оплаченный курс
    ...
    # Платежная система
    payment_method: models.CharField    # card, paypal, stripe
    transaction_id: models.CharField    # ID транзакции
...
### 6. LessonProgress (Прогресс изучения)
...
3. 🔲 **Course модель** с полями (в работе согласно заданию)
4. 🔲 **Lesson модель** с полями (в работе согласно заданию)
5. 🔲 **DRF API** для Course и Lesson
6. 🔲 **Базовые права доступа** (владелец vs обычный пользователь)

### Фаза 2: Монетизация

7. 🔲 Subscription модель
8. 🔲 Payment модель
9. 🔲 Интеграция Stripe
